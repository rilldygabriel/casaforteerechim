...
export async function POST(request: Request, { params }: { params: Promise<{ slug: string }> }) {
  try {
    ...
    const validationError = isPostEncounter ? validatePostEncounterRegistration(input) : isEncounter ? validateEncounterRegistration(input) : isBurger ? validateHamburgerRegistration(input) : validateRegistration(input);
    ...
    if (isBurger) {
      ...
      if (error?.code === "23505") {
        ...
        return NextResponse.json({ error: "Já existe uma reserva com este telefone para o Hambúrguer da Casa." }, { status: 409 });
      }
      if (error?.message?.includes("CAPACITY_EXCEEDED")) return NextResponse.json({ error: "Não há unidades suficientes disponíveis para este pedido. Diminua a quantidade e tente novamente." }, { status: 409 });
      if (error?.message?.includes("REGISTRATION_CLOSED")) return NextResponse.json({ error: "As reservas estão encerradas." }, { status: 409 });
    ...
    if (amountCents > 0) {
      ...
      try {
        ...
        return NextResponse.json({ accepted: true, paymentId, amountCents, message: isBurger ? "Pedido reservado. Conclua o pagamento para confirmar." : "Inscrição reservada. Conclua o pagamento para confirmar." }, { status: 201 });
      } catch (checkoutError) {
