...
export default async function AdminPage() {
  ...
  const [{ data: profile }, disciplerResult, leaderResult, connectMemberResult] = await Promise.all([
  ...
  if (!profile || (!isAdmin && (!isApproved || (!isDiscipler && ministryCount === 0 && !isConnectMember && !canManageFinance)))) {
    redirect("/familia");
  }
  ...
  return (
    <main className="admin-dashboard">
      ...
      <section className="admin-dashboard-grid" aria-label="Módulos do painel">
        ...
        {!isAdmin && isDiscipler && <Module number="02" href="/admin/agenda-pastoral" title="Agenda Pastoral" copy="Escolha um horário disponível para seu discipulado com os pastores." action="Ver horários livres" />}
