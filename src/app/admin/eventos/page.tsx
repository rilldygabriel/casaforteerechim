...
export default async function AdminEventsPage({ searchParams }: { searchParams: Promise<Search> }) {
  ...
  return <main className="admin-events-page">
    ...
    {filters.tab === "inscricoes" ? <RegistrationsTab events={allEvents} registrations={filtered} tickets={tickets ?? []} filters={filters} /> : <EventsTab events={allEvents} registrations={allRegistrations} editing={editing} canCreateEvents={scope.globalAccess} />}
  </main>;
}
...
function RegistrationsTab({ events, registrations, tickets, filters }: { events: EventRow[]; registrations: RegistrationRow[]; tickets: TicketRow[]; filters: Search }) {
