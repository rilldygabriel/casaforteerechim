...
type RegistrationRow = {
  id: string; event_id: string; full_name: string; phone: string; phone_normalized: string; attendance_duration: string;
  notes: string; status: string; created_at: string; events: { id?: string; title: string; slug?: string } | null;
};
...
export default async function AdminEventsPage({ searchParams }: { searchParams: Promise<Search> }) {
  ...
  const filtered = allRegistrations.filter((item) => {
    ...
    return (!filters.evento || item.event_id === filters.evento) && (!filters.status || item.status === filters.status) && (!filters.busca || item.full_name.toLowerCase().includes(textQuery) || item.phone_normalized.includes(query));
  });
