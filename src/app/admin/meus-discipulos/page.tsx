...
export default async function MyDisciplesPage() {
  ...
  const { data: meetingRequests } = relationshipIds.length ? await service.from("discipleship_scheduling_requests").select("relationship_id").in("relationship_id", relationshipIds).eq("status", "pending") : { data: [] };
  const requestRelationshipIds = new Set((meetingRequests ?? []).map((item) => item.relationship_id));
  ...
  return <main className="admin-visitors-page"><header className="admin-section-header"><Link href="/admin"><Image src="/images/logo-casa-forte.png" alt="Igreja Casa Forte" width={190} height={74} priority /></Link><nav><Link href="/admin">Voltar ao painel</Link></nav></header><section className="admin-visitors-hero"><p className="section-eyebrow"><span aria-hidden="true" />Meu discipulado</p><h1>Meus discípulos</h1><p>Somente as pessoas confiadas ao seu cuidado aparecem aqui.</p></section><section className="role-panel-list"><Link className="role-panel-person" href="/admin/agenda-pastoral"><span>Agenda de discipulado com os pastores</span><h3>Horários com Pr. Rilldy e Pra. Lize</h3><p>Veja os cartões disponíveis e reserve seu horário de discipulado pastoral.</p><strong>Abrir Agenda Pastoral →</strong></Link><article className="role-panel-card"><header><div><span>Acompanhamento pastoral</span><h2>Pessoas sob meu cuidado</h2></div><strong>{relationships?.length ?? 0} discípulo(s)</strong></header><div className="role-panel-people">{!relationships?.length ? <p className="role-panel-empty">Nenhum discípulo vinculado à sua conta ainda.</p> : relationships.map((relationship) => { const person = peopleById.get(relationship.disciple_id); const days = daysSince(latestByRelationship.get(relationship.id)); return <Link className="role-panel-person" href={`/familia/lideranca/discipulos/${relationship.id}`} key={relationship.id}>{requestRelationshipIds.has(relationship.id) && <em>Precisa de discipulado</em>}<span>Discípulo(a)</span><h3>{person?.full_name || person?.email || "Membro da Família"}</h3><p>{days === null ? "Sem discipulado registrado" : days === 0 ? "Discipulado realizado hoje" : `Último discipulado há ${days} ${days === 1 ? "dia" : "dias"}`}</p><strong>Abrir acompanhamento →</strong></Link>; })}</div></article></section></main>;
}
