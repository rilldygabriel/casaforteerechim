...
export default async function PaymentReturnPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const { data: payment } = reference
    ? await getSupabaseServiceClient().from("mercado_pago_payments").select("status,purpose,event_id,registration_id,amount_cents,tithe_cents,offering_cents,firstfruits_cents").eq("id", reference).maybeSingle()
    : { data: null };
  const approved = payment?.status === "approved";
  const { data: ticket } = approved && payment?.purpose === "event" && payment.registration_id
    ? await getSupabaseServiceClient().from("event_tickets").select("public_token").eq("registration_id", payment.registration_id).maybeSingle()
    : { data: null };
