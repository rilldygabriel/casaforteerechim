...
export default async function Generosidade() {
  const supabase = await getSupabaseServerClient();
  const { data: { user } } = await supabase.auth.getUser();
  const { data: profile } = user ? await supabase.from("member_profiles").select("full_name").eq("user_id", user.id).maybeSingle() : { data: null };
  return <main className="inner-page generosity-page">
    ...
    <MercadoPagoCheckout configured={isMercadoPagoBrickConfigured()} publicKey={process.env.NEXT_PUBLIC_MERCADO_PAGO_PUBLIC_KEY || ""} defaultName={profile?.full_name || ""} defaultEmail={user?.email || ""} />
    ...
    <PixCopyOptions />
    <p className="pix-security-note">Antes de concluir a transferência, confira os dados do favorecido no seu aplicativo bancário. Pix feitos diretamente pela chave não recebem classificação automática de dízimo, primícias ou oferta no painel.</p>
  </main>;
}
