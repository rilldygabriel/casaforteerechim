...
export default async function EventRegistrationPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className="event-public-page">
    ...
    <section className="event-registration-layout"><div><p className="home-kicker">Inscrição agora</p><h2>Faça sua inscrição</h2><p>{event.slug === "pos-encontro-agosto-2026" ? "Informe seus dados e confirme se você já participou do Encontro com Deus na Casa." : isEncounter ? "Informe nome completo, e-mail e WhatsApp. Depois escolha Pix ou cartão PagBank para confirmar sua vaga." : "Preencha os dados abaixo. Nossa equipe poderá entrar em contato pelo WhatsApp com as próximas orientações."}</p></div><RegistrationForm slug={event.slug} enabled={availability.open} feeCents={Number(event.registration_fee_cents)} variant={event.slug === "pos-encontro-agosto-2026" ? "post-encounter" : isEncounter ? "encounter" : "standard"} /></section>
  </main>;
}
