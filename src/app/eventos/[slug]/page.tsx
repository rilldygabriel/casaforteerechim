...
export default async function EventRegistrationPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className="event-public-page">
    ...
    <section className="event-public-hero">
      <div><p className="home-kicker">{event.category} · {event.status === "confirmed" ? "Confirmado" : "A confirmar"}</p><h1>{event.title}</h1><p>{event.description}</p>{event.image_url ? <figure className={`event-public-cover${isBurger ? " is-wide-banner" : ""}`}><Image src={event.image_url} alt={`Capa do evento ${event.title}`} fill priority={isBurger} sizes="(max-width: 850px) 100vw, 55vw" /></figure> : null}</div>
      <div className="event-public-summary"><dl><div><dt>Data</dt><dd>{formatDateRange(event.start_date, event.end_date)}</dd></div><div><dt>Horário</dt><dd>{isBurger ? "Após o culto" : `${formatTime(event.start_time)}${event.end_time ? ` às ${formatTime(event.end_time)}` : ""}`}</dd></div><div><dt>Local</dt><dd>{event.location || "Igreja Casa Forte Erechim"}</dd></div><div><dt>Valor</dt><dd>{isBurger ? "Simples R$ 20 · Duplo R$ 30" : Number(event.registration_fee_cents) > 0 ? `${new Intl.NumberFormat("pt-BR", { style: "currency", currency: "BRL" }).format(Number(event.registration_fee_cents) / 100)}${isEncounter ? " · cartão em até 4x" : ""}` : "Gratuito"}</dd></div><div><dt>Inscrições</dt><dd>{availability.label}{remaining !== null ? ` · ${remaining} ${isBurger ? "unidades" : "vagas"} restantes` : ""}</dd></div></dl><EventAttendanceControl event={{ id: `database-${event.id}`, title: event.title, startDate: event.start_date, endDate: event.end_date ?? undefined, startTime: event.start_time?.slice(0, 5) ?? undefined, category: "Eventos especiais", status: event.status === "cancelled" ? "cancelled" : event.status === "tentative" ? "tentative" : "confirmed" }} className="event-public-attendance" /></div>
    </section>
    <section className="event-registration-layout"><div><p className="home-kicker">Inscrição agora</p><h2>{isBurger ? "Faça sua reserva" : "Faça sua inscrição"}</h2><p>{event.slug === "pos-encontro-agosto-2026" ? "Informe seus dados e confirme se você já participou do Encontro com Deus na Casa." : isEncounter ? "Informe nome completo, e-mail e WhatsApp. Depois escolha Pix ou cartão Mercado Pago para confirmar sua vaga." : isBurger ? "Escolha quantos hambúrgueres simples e duplos desejar. Depois pague por Pix ou cartão em uma vez para confirmar o pedido." : "Preencha os dados abaixo. Nossa equipe poderá entrar em contato pelo WhatsApp com as próximas orientações."}</p></div><RegistrationForm slug={event.slug} enabled={availability.open} feeCents={Number(event.registration_fee_cents)} variant={event.slug === "pos-encontro-agosto-2026" ? "post-encounter" : isEncounter ? "encounter" : isBurger ? "burger" : "standard"} mercadoPagoPublicKey={process.env.NEXT_PUBLIC_MERCADO_PAGO_PUBLIC_KEY?.trim() || ""} /></section>
  </main>;
}
