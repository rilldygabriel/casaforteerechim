...
export default function RegistrationForm({ slug, enabled, feeCents = 0, variant = "standard", mercadoPagoPublicKey = "" }: { slug: string; enabled: boolean; feeCents?: number; variant?: "standard" | "post-encounter" | "encounter" | "burger"; mercadoPagoPublicKey?: string }) {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    const error = variant === "post-encounter" ? validatePostEncounterRegistration(form) : variant === "encounter" ? validateEncounterRegistration(form) : variant === "burger" ? validateHamburgerRegistration(form) : validateRegistration(form);
    ...
    try {
      ...
      const result = (await response.json()) as { error?: string; message?: string; accepted?: boolean; checkoutUrl?: string; paymentId?: string; amountCents?: number };
  ...
  return <form className="event-registration-form" onSubmit={submit}>
    ...
    {variant === "post-encounter" ? <fieldset className="event-eligibility"><legend>Você fez o Encontro com Deus na Casa?</legend><label><input required type="radio" name="completedEncounter" value="yes" checked={form.completedEncounter === "yes"} onChange={(event) => setForm({ ...form, completedEncounter: event.target.value, consent: true })} /> Sim, eu fiz</label><label><input required type="radio" name="completedEncounter" value="no" checked={form.completedEncounter === "no"} onChange={(event) => setForm({ ...form, completedEncounter: event.target.value, consent: true })} /> Não fiz</label></fieldset> : variant === "encounter" ? <p className="event-payment-notice">Valor: <strong>{new Intl.NumberFormat("pt-BR", { style: "currency", currency: "BRL" }).format(feeCents / 100)}</strong>. Pague por Pix ou cartão em até 4 vezes. Ao continuar, você autoriza o uso dos dados para organizar o evento e confirmar sua inscrição.</p> : variant === "burger" ? <><fieldset className="burger-options"><legend>Monte seu pedido</legend><label><span><strong>Hambúrguer simples</strong><small>R$ 20 cada</small></span><input aria-label="Quantidade de hambúrgueres simples" type="number" inputMode="numeric" min="0" max="100" step="1" value={form.simpleQuantity} onChange={(event) => setForm({ ...form, simpleQuantity: Math.max(0, Math.min(100, Number(event.target.value) || 0)) })} /></label><label><span><strong>Hambúrguer duplo</strong><small>R$ 30 cada</small></span><input aria-label="Quantidade de hambúrgueres duplos" type="number" inputMode="numeric" min="0" max="100" step="1" value={form.doubleQuantity} onChange={(event) => setForm({ ...form, doubleQuantity: Math.max(0, Math.min(100, Number(event.target.value) || 0)) })} /></label><div><span>Total do pedido</span><strong>{new Intl.NumberFormat("pt-BR", { style: "currency", currency: "BRL" }).format(burgerTotalCents / 100)}</strong></div></fieldset><p className="event-payment-notice">Pague por Pix ou cartão em <strong>1 vez</strong>, sem sair do site. A reserva é confirmada após a aprovação do pagamento.</p><label className="event-consent"><input required type="checkbox" checked={form.consent} onChange={(event) => setForm({ ...form, consent: event.target.checked })} /><span>Autorizo o uso dos dados exclusivamente para organizar minha reserva.</span></label></> : <><label>Há quanto tempo frequenta a Casa?<select required value={form.attendanceDuration} onChange={(event) => setForm({ ...form, attendanceDuration: event.target.value })}><option value="">Selecione uma opção</option>{ATTENDANCE_OPTIONS.map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select></label><label>Observações <small>Opcional</small><textarea maxLength={1500} rows={5} placeholder="Se desejar, conte algo importante para nossa equipe." value={form.notes} onChange={(event) => setForm({ ...form, notes: event.target.value })} /></label><label className="event-consent"><input required type="checkbox" checked={form.consent} onChange={(event) => setForm({ ...form, consent: event.target.checked })} /><span>Autorizo o uso das informações fornecidas exclusivamente para contato e organização deste evento.</span></label></>}
    {message ? <p className="event-form-message" data-state={state} role="status">{message}</p> : null}
    <button type="submit" disabled={state === "sending"}>{state === "sending" ? "Enviando…" : variant === "burger" ? `Reservar e pagar · ${new Intl.NumberFormat("pt-BR", { style: "currency", currency: "BRL" }).format(burgerTotalCents / 100)}` : feeCents > 0 ? `Inscrever e escolher pagamento · ${new Intl.NumberFormat("pt-BR", { style: "currency", currency: "BRL" }).format(feeCents / 100)}` : "Enviar inscrição"}</button>
  </form>;
}
