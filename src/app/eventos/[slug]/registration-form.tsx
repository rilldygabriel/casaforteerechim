...
export default function RegistrationForm({ slug, enabled, feeCents = 0, variant = "standard", mercadoPagoPublicKey = "", member = null }: { slug: string; enabled: boolean; feeCents?: number; variant?: "standard" | "post-encounter" | "encounter" | "burger"; mercadoPagoPublicKey?: string; member?: MemberIdentity | null }) {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    const error = variant === "post-encounter" ? validatePostEncounterRegistration(form) : variant === "encounter" ? validateEncounterRegistration(form) : variant === "burger" ? validateHamburgerRegistration({ phone: form.phone, simpleQuantity: form.simpleQuantity, doubleQuantity: form.doubleQuantity, isMember: Boolean(member) }) : validateRegistration(form);
    if (error) { setState("error"); setMessage(error); return; }
    setState("sending"); setMessage("Enviando sua inscrição…");
    try {
      const payload = variant === "burger" ? { phone: member ? "" : form.phone, simpleQuantity: form.simpleQuantity, doubleQuantity: form.doubleQuantity } : form;
  ...
  return <form className={`event-registration-form${variant === "burger" ? " is-burger" : ""}`} onSubmit={submit}>
    {variant !== "burger" ? <><label>Nome completo<input required minLength={3} maxLength={160} autoComplete="name" value={form.fullName} onChange={(event) => setForm({ ...form, fullName: event.target.value })} /></label><label>E-mail{feeCents > 0 ? null : <small> Opcional</small>}<input required={feeCents > 0} type="email" maxLength={254} autoComplete="email" value={form.email} onChange={(event) => setForm({ ...form, email: event.target.value })} /></label><label>Telefone ou WhatsApp<input required inputMode="tel" autoComplete="tel" placeholder="(54) 99999-9999" value={form.phone} onChange={(event) => setForm({ ...form, phone: event.target.value })} /></label></> : !member ? <label>Seu WhatsApp<input required inputMode="tel" autoComplete="tel" placeholder="(54) 99999-9999" value={form.phone} onChange={(event) => setForm({ ...form, phone: event.target.value })} /></label> : null}
