...
export default async function FamilyNotificationsPage() {
  ...
  const [{ data: announcements }, { data: reads }, { data: likes }, { data: comments }] = await Promise.all([
  ...
  return <main className="family-notifications-page">
    ...
    <section className="family-notifications-list">{announcements?.length ? announcements.map((item) => {
      ...
      return <article data-unread={!readIds.has(item.id)} key={item.id}><div><span>{readIds.has(item.id) ? "Lida" : "Nova"}</span><time>{new Intl.DateTimeFormat("pt-BR", { dateStyle: "long", timeStyle: "short", timeZone: "America/Sao_Paulo" }).format(new Date(item.created_at))}</time></div><h2>{item.title}</h2><p>{item.body}</p><AnnouncementInteractions announcementId={item.id} userId={user.id} liked={announcementLikes.some((like) => like.user_id === user.id)} likeCount={announcementLikes.length} comments={announcementComments} /></article>;
