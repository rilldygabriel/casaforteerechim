...
async function ensureProfile(user){
  ...
  if(data)return data;
  const initial={user_id:user.id,email:user.email||null,full_name:user.user_metadata?.full_name||'',church_role:'visitante',ministries:[],profile_completed:false};
  const {data:created,error}=await client.from('member_profiles').upsert(initial,{onConflict:'user_id'}).select().single();
